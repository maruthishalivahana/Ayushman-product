...
exports.convertToCSV = exports.buildFraudModelInstances = exports.cleanAndValidateData = exports.extractStructuredDataWithLLM = exports.extractTextFromDocument = exports.inspectUploadedFile = void 0;
...
const FEATURE_KEYS = Object.keys(FEATURE_PRIORS);
...
const inspectUploadedFile = (filePath, originalName, mimeType) => {
...
const extractTextFromDocument = async (filePath, originalName, mimeType) => {
    const inspection = (0, exports.inspectUploadedFile)(filePath, originalName, mimeType);
...
const extractStructuredDataWithLLM = async (rawText) => {
    if (!rawText?.trim()) {
...
${textForPrompt}`;
    ...
    if (geminiApiKey) {
        try {
            const geminiResponse = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${geminiApiKey}`, {
                systemInstruction: {
            ...
            if (axios_1.default.isAxiosError(error)) {
                ...
                modelErrors.push(`gemini-direct(${geminiModel}): ${error.message}`);
            }
        }
    }
    if (!openRouterApiKey) {
...
exports.extractStructuredDataWithLLM = extractStructuredDataWithLLM;
const cleanAndValidateData = (jsonData) => {
    const cleanedClaim = {
...
exports.cleanAndValidateData = cleanAndValidateData;
const deriveLengthOfStay = (claim) => {
...
const inferProbabilisticFallback = (field, current, claim) => {
    ...
    switch (field) {
        ...
        case 'Claim_Submitted_Late': {
            ...
            if (admissionType.includes('emergency'))
                return 0;
...
const buildFraudModelInstances = (cleanClaimData, llmData) => {
...
exports.buildFraudModelInstances = buildFraudModelInstances;
const convertToCSV = async (data, outputPath) => {
    const csvWriter = (0, csv_writer_1.createObjectCsvWriter)({
        ...
        append: fs_1.default.existsSync(outputPath)
    });
